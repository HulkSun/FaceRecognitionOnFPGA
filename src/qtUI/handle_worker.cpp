...
void HandleWorker::PlayStream(int frameNum)
{
    ...
    if(av_read_frame(pAVFormatContext, &pAVPacket) >= 0){   //获取一帧视频的压缩数据
...
//    qDebug() << QString::number(userId) << " | detect takes" << time.elapsed();
    ...
    for(int i = 0; i != numFace; ++i) {
        ...
        if(x < 0) x = 0;
        if(y < 0) y = 0;
        if(x + h > img.rows) h = img.rows - x - 1;
        if(y + w > img.cols) w = img.cols - y - 1;
        cv::Mat face;
...
//            emit ShowInTableSig(showPersonInfo, preFaceNum);
            ...
            // 保存图像，并写入数据库
            if (tim.elapsed() > 2000) {     // 与上次保存的时间超过1秒再保存
                QDir qdir;
                QString saveDir = "./saveData";
                if (!qdir.exists(saveDir)) qdir.mkdir(saveDir);
                QString fileDir = saveDir + "/" + QString::fromStdString(personInfo.name);
                if (!qdir.exists(fileDir)) qdir.mkdir(fileDir);
                QDateTime tempDataTime = QDateTime::currentDateTime();
                QString filePath = fileDir + "/" + QString::number(userId) + "_" + QString::fromStdString(personInfo.name)
                        + "_" + tempDataTime.toString("yyyy-MM-dd_hh:mm:ss") + ".jpg";
                pixmap.save(filePath);      /* 保存帧图像 */
                QString snapshotFilePath = fileDir +"/" + QString::number(userId) + "_" + QString::fromStdString(personInfo.name)
                        + "_" + tempDataTime.toString("yyyy-MM-dd_hh:mm:ss") + "_snapshot.jpg";
