...
class mihasher {
 ...
 public:
    ...
    void rangebatchquery(vector<vector<UINT32> >& results, qstat *stats, UINT8 *queries, UINT32 numq, int dim1queries);
   	
 private:
    void rangequery(vector<UINT32>& results, qstat *stats, UINT8 *query, UINT64 *chunks);

    void query(UINT32 *results, UINT32* numres, qstat *stats, UINT8 *q, UINT64 * chunks, UINT32 * res, int* cuda_chunks_counter, UINT64* cuda_index_filter);
};
