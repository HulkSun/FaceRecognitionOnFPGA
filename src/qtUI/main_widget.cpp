...
bool MainWidget::AddFaceDataSet(QString _dataSetDir)
{
    ...
    if (!storeDir->exists(facesDir)) storeDir->mkdir(facesDir);
    ...
    for (int id = 0; id < subFileList.count(); ++id) {
        if (subFileList[id].fileName() == QString(".") || subFileList[id].fileName() == QString("..")) continue;
        QPersonInfo qinfo;
        ...
        if (!storeDir->exists(idDir)) storeDir->mkdir(idDir);

        QDir subDataDir(subFileList[id].filePath());
        QStringList filter;
        filter << "*.tif" << "*.jpg" << "*.png" << "*.bmp";
        ...
        for (int k = 0; k < sub2FileList.count(); ++k) {
            if (sub2FileList[k].fileName() == QString(".") || sub2FileList[k].fileName() == QString("..")) continue;
            QString imgName = sub2FileList[k].fileName();
            //            qDebug() << imgName;
            if (imgName.left(1) == QString("F")) qinfo.sex = QString("女");
