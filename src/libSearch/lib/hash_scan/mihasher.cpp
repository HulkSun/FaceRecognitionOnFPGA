...
void mihasher::rangebatchquery(vector<vector<UINT32> >& results, qstat *stats, UINT8 *queries, UINT32 numq, int dim1queries)
{
    counter = new bitarray;
    counter->init(N);
    ...
    for (int i=0; i<numq; i++) {
		rangequery(results[i], pstats, pq, chunks);
...
void mihasher::rangequery(vector<UINT32>& results, qstat *stats, UINT8 *query, UINT64 *chunks)
{
