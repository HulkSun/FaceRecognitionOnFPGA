...
std::vector<FaceInfo> MTCNN::BoxRegress(std::vector<FaceInfo>& faceInfo,int stage){
	...
	for(unsigned int bboxId =0;bboxId<faceInfo.size();bboxId++){
		FaceRect faceRect;
...
void MTCNN::Padding(int img_w,int img_h){
	for(unsigned int i=0;i<regressed_rects_.size();i++){
		FaceInfo tempFaceInfo;
...
#endif
	/* Load the network. */
	PNet_.reset(new Net<float>((proto_model_dir+"/MTCNN_det1.prototxt"), TEST));
	PNet_->CopyTrainedLayersFrom(proto_model_dir+"/MTCNN_det1.caffemodel");
...
#ifdef CPU_ONLY
	RNet_.reset(new Net<float>((proto_model_dir+"/MTCNN_det2.prototxt"), TEST));
#else
	RNet_.reset(new Net<float>((proto_model_dir+"/MTCNN_det2_input.prototxt"), TEST));
#endif
	RNet_->CopyTrainedLayersFrom(proto_model_dir+"/MTCNN_det2.caffemodel");
...
#ifdef CPU_ONLY
	ONet_.reset(new Net<float>((proto_model_dir+"/MTCNN_det3.prototxt"), TEST));
#else
	ONet_.reset(new Net<float>((proto_model_dir+"/MTCNN_det3_input.prototxt"), TEST));
#endif
	ONet_->CopyTrainedLayersFrom(proto_model_dir+"/MTCNN_det3.caffemodel");
...
cv::Mat MTCNN::Align(const cv::Mat &img, const FacePts facePts)
{
	// cv::Mat face;
	// std::vector<cv::Point2f> ref_pts;
	// ref_pts.push_back(cv::Point2f(43.9736, 51.5082));
	// ref_pts.push_back(cv::Point2f(86.4005, 52.1159));
	// ref_pts.push_back(cv::Point2f(66.2167, 79.2851));
	// ref_pts.push_back(cv::Point2f(46.7114, 101.486));
	// ref_pts.push_back(cv::Point2f(82.7252, 101.673));
	// std::vector<cv::Point2f> pts(5, cv::Point2f(0, 0));
	// for(int i = 0; i < 5; ++i){
	// 	pts[i].x = facePts.y[i];
	// 	pts[i].y = facePts.x[i];
	// }
    // cv::Mat tform = cv::estimateRigidTransform(pts, ref_pts, false);
	// if(tform.rows == 2 && tform.cols == 3){
	// 	cv::warpAffine(img, face, tform, cv::Size(128, 128));
	// }
    // return face;   // 当estimateRigidTransform执行失败后，Align函数返回空矩阵

	std::vector<cv::Point2f> ref_pts;
	ref_pts.push_back(cv::Point2f(30.2946, 51.6963));
	ref_pts.push_back(cv::Point2f(65.5318, 51.5014));
	ref_pts.push_back(cv::Point2f(48.0252, 71.7366));
	ref_pts.push_back(cv::Point2f(33.5493, 92.3655));
	ref_pts.push_back(cv::Point2f(62.7299, 92.2041));
	std::vector<cv::Point2f> pts(5, cv::Point2f(0, 0));
	for (int i = 0; i < 5; ++i)
	{
		pts[i].x = facePts.y[i];
	...
	cv::Size image_size(96, 112);
	cv::Point2f ref_eye_center(
		0.5f * (ref_pts[1].x + ref_pts[0].x),
		0.5f * (ref_pts[1].y + ref_pts[0].y));
	double ref_dy = ref_pts[0].y - ref_pts[1].y;
	double ref_dx = ref_pts[0].x - ref_pts[1].x;
	double ref_len = std::sqrt(ref_dx * ref_dx + ref_dy * ref_dy);

	// Get the angle between the two eyes.
	cv::Point2f eye_center(0.5f * (pts[0].x + pts[1].x),
						   0.5f * (pts[0].y + pts[1].y));
	// Get the angle between the two eyes.
	...
	double scale = ref_len / len;
	cv::Mat rot_mat = cv::getRotationMatrix2D(eye_center, angle, scale);
	// Shift eye_center to ref_eye_center
	rot_mat.at<double>(0, 2) += ref_eye_center.x - eye_center.x;
	rot_mat.at<double>(1, 2) += ref_eye_center.y - eye_center.y;

	cv::Mat face;
	cv::warpAffine(img, face, rot_mat, image_size);
	return face;
}
